module.exports = {
    ...
    description: 'updating property seller.',
    ...
    fn: async function (inputs, exits) {
      ...
      try {
        // const checkSeller = await sails.models.propertyseller.findOne({
        //   property_id:inputs.property_id,
        //   user_id:!inputs.user_id,
        //   deleted_at: null
        // })
        // if(checkSeller){
        //   return exits.invalid({ status: false, data: [], message: 'Seller already exist for this property ' });
    ...
    //    const is_cda_sent =  inputs.is_cda_sent == 'true' ? 1 : 0;
        ...
        const getProp =  await sails.models.propertylisting.findOne({
          id:inputs.property_id,
          user_id:inputs.user_id,
          deleted_at: null
        })
        if(!getProp){
          return exits.success({ status: true, data: [], message: 'Property not found ' });

        }
       const obj = {
          property_type_id:inputs.property_type_id,
          property_id:getProp.id,
          user_id:getProp.user_id,
      ...
      //    is_cda_sent:is_cda_sent,
        ...
        const data = await PropertySeller.updateOne({id: inputs.id}).set(obj);
