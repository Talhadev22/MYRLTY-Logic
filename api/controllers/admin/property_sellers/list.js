module.exports = {
  ...
  fn: async function (inputs, exits) {
    ...
    try {
      ...
      if(fl.buyer){
        where += ` AND pb.seller_name like '%${fl.buyer}%'`;
      }
      if(fl.tcc){
    ...
    where
      ...
      // const findOptions = await sails.helpers.getOptions.with({ searchFrom: ['seller_name'], ...inputs });
      // const data = await sails.models.propertyseller.find(findOptions).sort('id DESC');;
      // for(obj of data){
      //   obj.is_cda_sent = Boolean(obj.is_cda_sent)
      //   obj.is_contract_to_lender = Boolean(obj.is_contract_to_lender)
      //   obj.is_earnest_money_received = Boolean(obj.is_earnest_money_received)
      ...
      //   obj.is_switch_over_utilities = Boolean(obj.is_switch_over_utilities)
      //  }
      // const count = await sails.models.propertyseller.count(findOptions.where);
      // return exits.adminResponse({ data: { data: data }, options: { routeName: 'property_sellers', range: JSON.parse(inputs.range), count: count } });
